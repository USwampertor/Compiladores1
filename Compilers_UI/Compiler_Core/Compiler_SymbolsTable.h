...
namespace CompilerCore
{
	...
	public:
		...
		std::string GetNodeType(std::string symbol, NODE_TYPE type);
		std::string GetSymbolType(std::string symbol, NODE_TYPE type, std::string functionName);
		...
		std::string NextString(Node* actualNode, NODE_TYPE type,std::string functionName);
	};
}
